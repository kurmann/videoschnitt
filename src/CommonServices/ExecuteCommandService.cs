...
namespace Kurmann.Videoschnitt.CommonServices
{
    public class ExecuteCommandService
    {
        ...
        public async Task<Result> ExecuteCommandAsync(string commandPath, string arguments)
        {
            ...
            using (var process = new Process { StartInfo = psi, EnableRaisingEvents = true })
            {
                ...
                if (process.ExitCode != 0)
                {
                    ...
                    return Result.Failure($"Command '{commandPath} {arguments}' failed with exit code {process.ExitCode}: {errorMessage}");
                }
            }
